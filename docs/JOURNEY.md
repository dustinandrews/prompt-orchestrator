...
## Phase 17: Installable Package (2026-05-31)

**Goal:** Turn prompt-orchestrator into a proper pip-installable CLI tool that works with existing projects.

### What Changed

**New package structure:**
- `prompt_orchestrator/` — pip-installable Python package
  - `cli.py` — three subcommands: `init`, `run`, `new`
  - `runner.py` — workflow executor (moved from `.setup/run_steps.py`)
  - `scaffold/` — command files, review templates, constitution (moved from `template/`)
  - `project_skeleton/` — skeleton files for `new` command (src/, tests/, pyproject.toml)

**CLI commands:**
- `prompt-orchestrator init` — scaffolds `.orchestrator/`, `steps.yaml`, and `._agents_not_allowed/` into any existing project directory
- `prompt-orchestrator run` — finds steps.yaml in cwd and executes the workflow with full retry/verification logic
- `prompt-orchestrator new --name X --spec Y` — creates a full project from skeleton (replaces old `setup.py`)

**Backward compatibility:** Old `.setup/setup.py` and `.setup/run_steps.py` are preserved. Existing projects generated by the old workflow still work.

**pyproject.toml:** Updated with proper package layout, entry point `prompt-orchestrator`, and `pyyaml` dependency.

### Tests

9 tests total — 7 existing (updated for new filenames) + 2 new for CLI subcommands. All pass.
